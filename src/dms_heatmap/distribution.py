...
All three classes share one set of bin edges, so the bars are comparable
across classes and not just within one.
"""
...
__all__ = ["ClassSeries", "DistributionStyle", "class_series", "render_distribution"]
...
def render_distribution(
    ...
    _add_note(fig, edges, clipped, style, fig_height)
    return fig, series
...
def _add_note(fig, edges, clipped: int, style, fig_height) -> None:
    ...
    fig.text(
        style.margin_left / style.width, 0.10 / fig_height,
        "  ".join(parts),
